...
property int mediaWorkspaceId: -1
        ...
        function resolveMediaWorkspace(rawJson) {
            ...
            if (!player) { mediaWorkspaceId = -1; return }
            const needle = String(player.identity || "").toLowerCase().trim()
            if (needle === "") { mediaWorkspaceId = -1; return }
            try {
                ...
                for (let i = 0; i < clients.length; i++) {
                    ...
                    if (cls.indexOf(needle) >= 0 || needle.indexOf(cls) >= 0
...
sourceComponent: Component {
                    ExpandedPlayerLayer {
                        ...
                        cavaLevels: islandContainer.cavaLevels  // add this
...
Loader {
                id: searchLoader
                anchors.fill: parent
                active: islandContainer.searchLayerVisible
                asynchronous: false
                visible: active
