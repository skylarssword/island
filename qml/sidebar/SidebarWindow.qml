...
PanelWindow {
    ...
    // ── Settings polling (appearance JSON) ───────────────────────────
    Process {
        id: settingsQuery
    ...
    Timer {
        ...
        onTriggered: if (!settingsQuery.running) settingsQuery.running = true
    }
    ...
    Connections {
        ...
        function onBrightnessSetFinished(value, success, errorString) {
            if (success) root.currentBrightness = value
        }
        function onVolumeSetFinished(value, success, errorString) {
            if (success) root.currentVolume = value
        }
    }
